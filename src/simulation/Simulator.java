...
public class Simulator implements WorldListener {
	...
	public void assignAddress(Simulatable s, int x, int y) {
		((CommandCenter)this.emergencyService).getGui().assignAddress(s, x, y);
	...
	public void nextCycle() {
		...
		for (int i = 0; i < plannedDisasters.size(); i++) {
			...
			if (d.getStartCycle() == currentCycle) {
				...
				i--;
				((GameGUI)((CommandCenter)this.emergencyService).getGui()).logDisaster(d);
		...
		for (int i = 0; i < buildings.size(); i++) {
			...
			if (b.getFireDamage() >= 100) {
				...
				((GameGUI)((CommandCenter)this.emergencyService).getGui()).logDisaster(c);
			}
		}
	...
	private void handleGas(Disaster d) {
		...
		if (b.getFireDamage() != 0) {
			...
			try{c.strike();}
				...
				System.out.println("CitizenAlreadyDeadException"+e.getMessage());
			}
			catch(BuildingAlreadyCollapsedException e){
				System.out.println("BuildingAlreadyCollapsedException"+e.getMessage());
			}
			executedDisasters.add(c);
		} else {
			try{d.strike();}
